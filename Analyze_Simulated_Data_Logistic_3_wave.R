analyze_simulated_data_logistic_3_wave <- function(sim_data_wide, 
  ...
  gee_formula_3_wave <- Y ~  post + last + 
    post:A1 + post:A2 + 
    last:A1 +last:A2 + post:A1:A2 + last:A1:A2;
  contrast_codes_for_regimens_last_wave <- cbind(1, # intercept;
                                                 1, # post;
                                                 1, # last;
                                                 regimens_1wave[,2:3], # post-0 dummy code interaction with A1 and A2;
                                                 regimens_1wave[,2:3], # last-wave dummy code interaction with A1 and A2;
                                                 regimens_1wave[,4], # post*A1*A2,
                                                 regimens_1wave[,4]); # last*A1*A2;
  gee_ind <- geeglm(formula = gee_formula_3_wave,
                             id=id,
                             weights = known_weight, family=binomial(),
                             data=data_for_analysis_long,
                             corstr = "independence" );
  final_logodds_estimates_ind <-
     drop(contrast_codes_for_regimens_last_wave %*% gee_ind$geese$beta);
   target_contrast_ind <-
     final_logodds_estimates_ind[regimen1] -
     final_logodds_estimates_ind[regimen2] ;
   variance_of_target_contrast_ind <-
     dummy_contrast_coefficients %*% contrast_codes_for_regimens_last_wave %*%
     gee_ind$geese$vbeta %*%
     t(contrast_codes_for_regimens_last_wave) %*% dummy_contrast_coefficients;
   ...
   gee_ar1 <- geeglm(formula = gee_formula_3_wave,
                     id=id,
   ...
   final_logodds_estimates_ar1 <-
     drop(contrast_codes_for_regimens_last_wave %*% gee_ar1$geese$beta);
   target_contrast_ar1 <-
   ...
   work_corr_as_zcor <- fixed2Zcor(cor.fixed=work_corr_exch,
                                   id=data_for_analysis_long$id,
                                   waves=data_for_analysis_long$total_wave);
   gee_exch <- geeglm(formula = gee_formula_3_wave,
                              id=id,
                              weights = known_weight,
                              data=data_for_analysis_long,
                              corstr = "fixed",
   ...
   final_logodds_estimates_exch <-
     drop(contrast_codes_for_regimens_last_wave %*% gee_exch$geese$beta);
