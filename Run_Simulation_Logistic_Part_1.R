...
for (pre_post_corr in c("no",
  ...
  for (effect_size in c("low",
    ...
    for (n_subjects  in c(300,500)) {
      beta_Y1_y0 <- switch(pre_post_corr,
                           "no"=0,
                           "low"=1.2,
                           "high"=3);
      if (pre_post_corr=="no") {
        beta_Y1_a1 <- switch(effect_size,
                             "low"=.10,
                             "medium"=.25,
                             "high"=.46);  # slight change from .47 on 7/29/22;
        beta_Y1_intercept <- -0.44;  
      }
      ...
      if (pre_post_corr=="high") {
        beta_Y1_a1 <- switch(effect_size,
                             ...
                             "medium"=.45,
                             "high"=.78);  
        beta_Y1_intercept <- -1.55;
      }
      E_Y0 <- .4;
      theta_R <- c(intercept = -.62, 
                   y0 = theta_R_y0,
                   a1 = theta_R_a1);
      beta_Y1 <- c(intercept = beta_Y1_intercept,
                   y0 = beta_Y1_y0,
                   r = beta_Y1_r,
                   a1 = beta_Y1_a1,
                   a2NR = 0,
                   a1a2NR = 0);
      ##################################################################
      # Calculate marginal probabilities for R and Y1:
      ##################################################################
      ...
      print(marg_E_R); 
      design_Y1 <- expand.grid(a2=c(-1,1),
                               a1=c(-1,1),
                               r=c(0,1),
                               y0=c(0,1),
                               intercept=1)[,5:1];
      design_Y1$a2NR <- (1-design_Y1$r)*design_Y1$a2; # code responders as zero here;
      design_Y1$a1a2NR <- (1-design_Y1$r)*design_Y1$a1*design_Y1$a2; 
      these_columns <- c("intercept","y0","r","a1","a2NR","a1a2NR");
      cond_E_Y1 <- plogis(as.matrix(design_Y1[,these_columns]) %*%
                            beta_Y1[these_columns]);
      print("Calculated conditional expected values for Y1 by path:")
      print(cbind(design_Y1,cond_E_Y1));
      design_Y1_marg <- expand.grid(a1=c(-1,1),
                                    a2NR=c(-1,1));
      regimen_names <- paste( ifelse(design_Y1_marg$a1>0,"p","m"), 
                              ifelse(design_Y1_marg$a2NR>0,"p","m"), 
                              sep="");
      marg_E_Y1 <- rep(0,4);
      names(marg_E_Y1) <- regimen_names;
      for (row in 1:4) {
        a1 <- design_Y1_marg$a1[row];
        a2 <- design_Y1_marg$a2NR[row];
        for (y0 in c(0,1)) {
          for (r in c(0,1)) {
            ...
            p_this_r_value <- ifelse(r==1, 
                                     this_cond_E_R, 
                                     1-this_cond_E_R);
            this_cond_E_Y1 <- cond_E_Y1[which(design_Y1$y0==y0 &
                                                design_Y1$r==r &  
                                                design_Y1$a1==a1 &
                                                design_Y1$a2==a2)];
            marg_E_Y1[row] <- marg_E_Y1[row] + 
              p_this_y0_value * p_this_r_value * this_cond_E_Y1;
          }
        }
      }
      print("Calculated regimen marginal expected values for Y1:");
      print(marg_E_Y1);
      ##################################################################
      ...
      cor_Y0_Y1 <- rep(NA, n_sims);
      cor_R_Y1 <- rep(NA, n_sims);
      mean_R <- rep(NA, n_sims);
      mean_Y1 <- rep(NA, n_sims);
      ...
      ######################################################
      print(paste(theta_R_y0,beta_Y1_y0,beta_Y1_r));
      ##################################################################
      ...
      for (this_sim in 1:n_sims) { 
        ...
        target_contrasts[this_sim,] <- 
          c(analysis_results$target_contrast_1_wave,
            ...
            analysis_results$target_contrast_adjusted,
        ...
        std_err_target_contrasts[this_sim,] <- 
          c(analysis_results$std_err_target_contrast_1_wave,
            ...
            analysis_results$std_err_target_contrast_adjusted,
        ...
        mean_Y1[this_sim] <- mean(sim_wide$Y1);
        cor_Y0_R[this_sim] <- cor(sim_wide)["Y0","R"]; 
        cor_Y0_Y1[this_sim] <- cor(sim_wide)["Y0","Y1"];
        cor_R_Y1[this_sim] <- cor(sim_wide)["R","Y1"];
      }
      ##################################################################
      ...
      p1 <- marg_E_Y1[regimen1]; #P(Y_post=1) for regimen 1 of pairwise comparison;
      p2 <- marg_E_Y1[regimen2]; #P(Y_post=1) for regimen 2 of pairwise comparison;
      ...
      n_required_margprob <- as.numeric(ceiling(((zQ+z_one_minus_half_alpha)^2)*
                                                  (var1_margprob_1_wave)/
                                                  (log(OR)^2)));  
      print("Predicted required sample size for .80 power:");
      print(n_required_margprob);
      ###################################################
      ...
      # given each regimen:
      design_for_responders <- design_Y1[which(design_Y1$r==1 & design_Y1$a2==-1),
                                         c("y0","r","a1")];
      # a2 is irrelevant here (because a2NR=0*a2) so we just pick one value
      E_Y1_for_responders <- cond_E_Y1[which(design_Y1$r==1 & design_Y1$a2==-1)];
      marg_E_Y1_for_responders <- c(
        m=(1-E_Y0)*E_Y1_for_responders[which(design_for_responders$y0==0 &
                                               design_for_responders$a1==-1)] + 
          E_Y0 * E_Y1_for_responders[which(design_for_responders$y0==1 &
                                             design_for_responders$a1==-1)],
        p=(1-E_Y0)*E_Y1_for_responders[which(design_for_responders$y0==0 &
                                               design_for_responders$a1==+1)] + 
          E_Y0 * E_Y1_for_responders[which(design_for_responders$y0==1 &
                                             design_for_responders$a1==+1)]) 
      # = E(Y1 | R=1, A1=a1) marginal over Y0;
      pPOSTr <- c(  mm=as.numeric(marg_E_Y1_for_responders["m"]),
                    mp=as.numeric(marg_E_Y1_for_responders["m"]),
                    pm=as.numeric(marg_E_Y1_for_responders["p"]),
                    pp=as.numeric(marg_E_Y1_for_responders["p"]))
      # = E(Y1|A1) for each regimen, which involves duplicating rows 
      ...
      design_for_nonresponders <- design_Y1[which(design_Y1$r==0),];
      E_Y1_for_nonresponders <- cond_E_Y1[which(design_Y1$r==0)];
      marg_E_Y1_for_nonresponders <- c(
        mm=(1-E_Y0)*E_Y1_for_nonresponders[which(design_for_nonresponders$y0==0 &
                                                   design_for_nonresponders$a1==-1 &
                                                   design_for_nonresponders$a2NR==-1)] + 
          E_Y0 * E_Y1_for_nonresponders[which(design_for_nonresponders$y0==1 &
                                                design_for_nonresponders$a1==-1 &
                                                design_for_nonresponders$a2NR==-1)],
        mp=(1-E_Y0)*E_Y1_for_nonresponders[which(design_for_nonresponders$y0==0 &
                                                   design_for_nonresponders$a1==-1 &
                                                   design_for_nonresponders$a2NR==+1)] + 
          E_Y0 * E_Y1_for_nonresponders[which(design_for_nonresponders$y0==1 &
                                                design_for_nonresponders$a1==-1 &
                                                design_for_nonresponders$a2NR==+1)],
        pm=(1-E_Y0)*E_Y1_for_nonresponders[which(design_for_nonresponders$y0==0 &
                                                   design_for_nonresponders$a1==+1 &
                                                   design_for_nonresponders$a2NR==-1)] + 
          E_Y0 * E_Y1_for_nonresponders[which(design_for_nonresponders$y0==1 &
                                                design_for_nonresponders$a1==+1 &
                                                design_for_nonresponders$a2NR==-1)],
        pp=(1-E_Y0)*E_Y1_for_nonresponders[which(design_for_nonresponders$y0==0 &
                                                   design_for_nonresponders$a1==+1 &
                                                   design_for_nonresponders$a2NR==+1)] + 
          E_Y0 * E_Y1_for_nonresponders[which(design_for_nonresponders$y0==1 &
                                                design_for_nonresponders$a1==+1 &
                                                design_for_nonresponders$a2NR==+1)])
      p1nr <- marg_E_Y1_for_nonresponders[regimen1];
      p2nr <- marg_E_Y1_for_nonresponders[regimen2];
      ...
      n_required_condprob <- as.numeric(ceiling(((zQ+z_one_minus_half_alpha)^2)*
                                                  (var1_condprob_1wave)/
                                                  (log(OR)^2)));  
      print("Predicted required sample size for .80 power:");
      print(n_required_condprob);
      ##################################################################
      ...
      vPOST <- marg_E_Y1 * (1-marg_E_Y1); # marginal variance from formula
      ...
      prob_R <- (r1+r2)/2;
      dummy_contrast_coefficients <- 0*marg_E_Y1;
      dummy_contrast_coefficients[regimen1] <- +1;
      dummy_contrast_coefficients[regimen2] <- -1;
      # assuming regimen 1 is (+,-) and regimen 2 is (-,-)
      var1_contrast_margprob_2_wave <- as.numeric( (4-2*prob_R) * c(0,dummy_contrast_coefficients) %*%
                                                     inv_bread_matrix_two_waves %*% c(0,dummy_contrast_coefficients)) ;
      predicted_power_margprob_2_wave <- 
        pnorm(sqrt(n_subjects*(log(OR)^2)/var1_contrast_margprob_2_wave) -
                z_one_minus_half_alpha); 
      print("Predicted power, correlated, from marginal probabilities:");
      print(predicted_power_margprob_2_wave);
      n_required_margprob_2_wave <- as.numeric(ceiling(((zQ+z_one_minus_half_alpha)^2)*
                                                         (var1_contrast_margprob_2_wave)/
                                                         (log(OR)^2)));  
      print("Predicted required sample size for .80 power:");
      print(n_required_margprob_2_wave);
      ###################################################
      # Two-wave power formula using conditional probabilities: 
      vPOSTnr <- marg_E_Y1_for_nonresponders*(1-marg_E_Y1_for_nonresponders);  
      ...
      var1_contrast_condprob_2_wave <- as.numeric( c(0,dummy_contrast_coefficients) %*%
                                                  inv_bread_matrix_two_waves %*% arrowhead_meat_matrix  %*%
                                                  inv_bread_matrix_two_waves %*% c(0,dummy_contrast_coefficients)) ;
      predicted_power_condprob_2_wave <-  
        pnorm(sqrt(n_subjects*(log(OR)^2)/var1_contrast_condprob_2_wave) -
                z_one_minus_half_alpha);  
      ...
      n_required_condprob_2_wave <- as.numeric(ceiling(((zQ+z_one_minus_half_alpha)^2)*
                                                         (var1_contrast_condprob_2_wave)/
                                                         (log(OR)^2)));  
      print("Predicted required sample size for .80 power:");
      print(n_required_condprob_2_wave);
      ##################################################################
      ...
      overall_results <- rbind(overall_results,
                               data.frame(OR=OR,
                                          ...
                                          beta_Y1_a1=beta_Y1_a1,  
                                          beta_Y1_y0=beta_Y1_y0, 
                                          ...
                                          mean_Y1=round(mean(mean_Y1),2),
                                          cor_Y0_R=round(mean(cor_Y0_R),2),
                                          cor_Y0_Y1=round(mean(cor_Y0_Y1),2),
                                          cor_R_Y1=round(mean(cor_R_Y1),2),
...
write.csv(x=overall_results,file="results-step-1.csv");