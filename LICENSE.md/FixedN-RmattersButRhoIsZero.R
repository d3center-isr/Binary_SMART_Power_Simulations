...
for (effect_size_scenario in 1:n_effect_size_coefficients) {
  ...
  #######################################################################################
  true_cell_conditional_means <- c(pre_0  = .90,  # baseline, future nonresponders;
                                   pre_1  = .50,  # baseline, future responders;
...
save.image(paste("Ran-RmattersButRhoIsZero-",as.integer(Sys.time()),".rdata",sep=""));
