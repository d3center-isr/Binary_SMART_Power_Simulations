...
for (effect_size_scenario in 1:3) {
  for (true_corr_scenario in 1:3) {
    ...
    var1_log_OR_pretty <- 2*(2-prob_R)*((4-3*rho^2)/(4*p1*(1-p1)) -
                                            (rho^2)/(2*sqrt(p1*(1-p1)*p2*(1-p2))) + 
                                            (4-3*rho^2)/(4*p2*(1-p2)));
    
    n_required_two_waves_pretty <-  as.numeric(ceiling(((zQ+z_one_minus_half_alpha)^2)*
                                                         (var1_log_OR_pretty)/
                                                         (log_OR^2)));
    ...
    these_answers <- data.frame(
      ...
      two_waves_pretty = n_required_two_waves_pretty,
